...
**更新** 本次更新反映了脚本生成器的重大增强，包括新的LLM服务架构、场景证据直接生成能力、改进的错误处理机制和完整的输出验证系统。
...
- generate_narration_from_scene_evidence：新增功能，直接从场景证据生成脚本，支持事实抽取与文案润色双阶段
...
**更新** 新增了 `generate_narration_from_scene_evidence` 函数和统一LLM服务架构，显著增强了脚本生成的灵活性和可靠性。
...
**更新** 新架构引入了统一LLM服务接口和输出验证机制，提供更强的错误处理能力和格式保证。
...
- 处理流程
  - 将场景证据转换为Markdown格式
  - 使用FACT_PROMPT进行事实抽取，生成事实版items
  - 使用POLISH_PROMPT进行文案润色，生成最终脚本
  - 兜底策略：_fallback_from_scene_evidence
- 输出
  - JSON脚本items，包含_id、timestamp、picture、narration、OST等字段
- 质量控制
...
**新增功能** 这是本次更新的重要新增功能，允许直接从场景证据生成脚本，无需复杂的帧分析过程。

```mermaid
flowchart TD
Start(["开始"]) --> Convert["scene_evidence_to_markdown"]
...
**图表来源**
- [app/services/generate_narration_script.py:123-155](file://app/services/generate_narration_script.py#L123-L155)
- [app/services/generate_narration_script.py:66-110](file://app/services/generate_narration_script.py#L66-L110)
- [app/services/script_fallback.py:45-58](file://app/services/script_fallback.py#L45-L58)

**章节来源**
- [app/services/generate_narration_script.py:123-155](file://app/services/generate_narration_script.py#L123-L155)
- [app/services/generate_narration_script.py:51-63](file://app/services/generate_narration_script.py#L51-L63)
- [app/services/generate_narration_script.py:66-110](file://app/services/generate_narration_script.py#L66-L110)
...
**更新** 新架构引入了统一LLM服务接口和输出验证机制，提供更强的错误处理能力。
...
**更新** 本次重大更新引入了全新的LLM服务架构，显著提升了系统的稳定性和可靠性。
...
- **新增** 直接从场景证据生成脚本
  - generate_narration_from_scene_evidence/scene_evidence/api_key/base_url/model，支持事实抽取与文案润色双阶段

**更新** 新增了从场景证据直接生成脚本的功能示例。
