...
graph TB
subgraph "服务层"
SN["subtitle_normalizer.py<br/>SRT解析/清洗/导出"]
...
## 核心组件
- 字幕解析与清洗：负责SRT文件解析、时间戳转换、文本清洗与段落规范化。
...
participant SN as "标准化器(subtitle_normalizer)"
...
### 字幕解析与清洗（subtitle_normalizer）
- SRT时间解析与转换：支持“HH:MM:SS,mmm”格式，提供秒级转换与回转。
- 段落解析：按空行分割块，提取序号、时间行与正文，统一为内部结构。
- 文本清洗：去除多余空白、首尾标点、统一标点集合。
- 段落规范化：合并相邻片段、控制最大字符数与时长、最小显示时长补足。
- 导出SRT：统一写出标准SRT格式。

```mermaid
flowchart TD
Start(["开始: 读取SRT文件"]) --> Parse["解析块/时间行/正文"]
Parse --> CleanText["清洗文本<br/>空白/标点/首尾处理"]
CleanText --> Merge["合并策略<br/>间距/长度/标点阈值"]
Merge --> Duration["时长校验<br/>最小显示时长"]
Duration --> ReID["重编号/生成seg_id"]
ReID --> Dump["导出SRT"]
Dump --> End(["结束"])
```
...
- 文本清洗与合并：线性扫描与简单拼接，复杂度低；合并策略中的阈值控制可减少无效合并。
...
## 结论
NarratoAI的字幕标准化器通过“解析-解码-清洗-合并-渲染”的完整链路，实现了跨平台、多来源的字幕统一。其核心优势在于：
- 统一SRT格式与时间轴，提升兼容性；
- 自动编码检测与替换，降低乱码风险；
- 可配置的段落合并与渲染参数，满足多样化需求。

## 附录

### 实际应用场景与处理效果示例
- 多来源SRT拼接：将多个片段的SRT按编辑时间范围对齐，统一输出为连续SRT，便于后续渲染。
- ASR转SRT清洗：将ASR生成的SRT进行标点、空白与时长规范化，显著提升可读性。
- 跨平台兼容：自动处理UTF-16/GBK/GB2312等编码，消除平台差异带来的解析问题。
- 字幕样式统一：通过WebUI参数统一颜色、字号、描边与位置，保证视频字幕一致性。
