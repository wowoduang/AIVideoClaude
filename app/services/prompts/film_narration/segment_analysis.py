...
参数：global_bible / prev_summary / subtitles / start_time / end_time
     / frame_descriptions / narrative_warnings
输出：segment_card JSON（含 real_narrative_state / plot_function 等）
"""
...
class SegmentAnalysisPrompt(ParameterizedPrompt):
    """第二轮：分段精理解，生成 segment_card"""
    ...
    def __init__(self):
        metadata = PromptMetadata(
            ...
            version="v1.0",
            description="第二轮LLM调用：对单个剧情段做精理解，区分字幕字面含义与真实叙事状态，输出 segment_card",
            model_type=ModelType.TEXT,
            output_format=OutputFormat.JSON,
            tags=["影视解说", "分段理解", "叙事分析", "real_narrative_state", "plot_function"],
            parameters=["global_bible", "prev_summary", "subtitles",
                        "start_time", "end_time", "frame_descriptions", "narrative_warnings"],
        )
        super().__init__(metadata, required_parameters=["subtitles", "start_time", "end_time"])
