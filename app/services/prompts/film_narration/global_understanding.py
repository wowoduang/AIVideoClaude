...
参数：subtitle_content
输出：global_story_bible JSON（含 narrative_warnings 雷区标记）
"""
...
class GlobalUnderstandingPrompt(ParameterizedPrompt):
    ...
    def __init__(self):
        metadata = PromptMetadata(
            ...
            version="v1.0",
            description="第一轮LLM调用：分析完整字幕生成全局剧情底稿，标注字幕不可信位置（narrative_warnings）",
            model_type=ModelType.TEXT,
            output_format=OutputFormat.JSON,
            tags=["影视解说", "剧情理解", "全局分析", "narrative_warnings", "雷区标记"],
            parameters=["subtitle_content"],
        )
        super().__init__(metadata, required_parameters=["subtitle_content"])
...
## 重要约束
1. narrative_warnings 必须标注所有字幕字面含义与真实叙事状态可能不符的位置
   - 角色说谎、试探、反讽、隐瞒时必须标注
   - 回忆/闪回导致时间层变化时必须标注
   - 旁白不代表当下剧情状态时必须标注
2. 不确定时输出"无法确定"，禁止脑补
