...
- [generate_short_summary.py](file://webui/tools/generate_short_summary.py)
...
## 更新摘要
**变更内容**
- 新增短脚本生成功能，支持短剧混剪和短剧解说两种模式
- 新增字幕文件上传功能，支持多种字幕格式（SRT、ASS、SSA、VTT）
- 新增实时进度跟踪机制，提供详细的生成过程反馈
- 新增字幕优先管线，支持证据预览和成本估算
- 更新脚本设置面板，集成多种生成模式
...
- 新增功能：短脚本生成（短剧混剪/短剧解说）、字幕文件上传、实时进度跟踪。
...
- 新增的短脚本生成功能通过 webui/tools 下的专用工具模块实现，支持实时进度跟踪。
...
A --> M["generate_script_short.py<br/>短脚本生成工具"]
A --> N["generate_short_summary.py<br/>短剧解说工具"]
A --> O["subtitle_text.py<br/>字幕处理工具"]
```
...
- [generate_script_short.py:20-284](file://webui/tools/generate_script_short.py#L20-L284)
- [generate_short_summary.py:139-297](file://webui/tools/generate_short_summary.py#L139-L297)
- [subtitle_text.py:69-124](file://app/services/subtitle_text.py#L69-L124)
...
- **新增** 短脚本生成（Short Script Generation）：支持短剧混剪和短剧解说两种模式，提供实时进度跟踪和字幕文件上传功能。
...
- **更新** 脚本来源：文件选择/上传、AI 自动生成（画面解说/短剧混剪/短剧解说）、从本地文件加载。
- **新增** 短剧混剪模式：支持自定义片段数量，使用共享底座 + 短剧风格策略，跳过关键帧提取和视觉分析。
- **新增** 短剧解说模式：要求高质量短剧字幕（支持 SRT/ASS/VTT），使用字幕优先管线，提供证据预览和成本估算。
- **新增** 字幕文件上传：支持多种字幕格式，自动编码检测与转换，提供实时上传反馈。
- **新增** 实时进度跟踪：使用进度条和状态文本显示生成过程，提供详细的步骤反馈。
...
- 使用字幕优先管线时，可查看证据预览面板了解生成过程的成本估算。
...
- [script_settings.py:326-402](file://webui/components/script_settings.py#L326-L402)
...
### **新增** 短脚本生成（Short Script Generation）

#### 短剧混剪模式（Short Drama Editing）
- **新增** 生成流程：严格验证视频和字幕文件，使用共享底座 + 短剧风格策略。
- **新增** 实时进度跟踪：使用进度条和状态文本显示各步骤进度。
- **新增** 字幕处理：字幕标准化、场景切分、证据融合，跳过关键帧提取和视觉分析。
- **新增** 参数配置：支持自定义片段数量，默认为5。

#### 短剧解说模式（Short Drama Summary）
- **新增** 字幕优先管线：要求高质量短剧字幕（支持 SRT/ASS/VTT）。
- **新增** 证据预览：提供成本估算和全局摘要，支持证据预览面板。
- **新增** 回退机制：优先使用字幕优先管线，失败时回退到旧的 LLM 分析方式。
- **新增** JSON解析修复：自动解析和修复LLM生成的JSON格式问题。

#### 字幕文件上传功能
- **新增** 多格式支持：SRT、ASS、SSA、VTT格式。
- **新增** 编码检测：自动检测和转换字幕编码（UTF-8、UTF-16、GBK、GB2312）。
- **新增** 实时反馈：上传成功后显示编码信息和文件大小。
- **新增** 安全处理：防止路径注入攻击，自动清理文件名。

#### 实时进度跟踪
- **新增** 进度条显示：使用st.progress()显示整体进度。
- **新增** 状态文本：使用st.empty()显示详细状态信息。
- **新增** 步骤反馈：每个处理步骤都有对应的进度和状态提示。

最佳实践
- 短剧混剪模式适合已有视频素材但需要快速生成脚本的场景。
- 短剧解说模式适合已有高质量字幕内容的短剧制作。
- 字幕文件建议使用UTF-8编码，确保兼容性和处理效率。
- 实时进度跟踪有助于监控长时间生成任务的状态。
...
- 组件耦合
  ...
  - **新增** 短脚本生成工具通过 webui/tools 下的专用模块实现，与主界面通过函数调用连接。
- 外部依赖
  - LLM 提供商通过 LiteLLM 统一接入，支持多家提供商与 Base URL 自定义。
  - FFmpeg 工具用于硬件加速检测与视频处理，诊断组件提供可视化配置与故障排除。
  - **新增** 字幕处理依赖 app/services/subtitle_text.py 提供跨平台字幕解码功能。
- 配置来源
...
SS --> GSS["generate_script_short.py"]
SS --> GSSUM["generate_short_summary.py"]
GSS --> ST["subtitle_text.py"]
GSSUM --> ST
```
...
- **新增** 短脚本生成性能：短剧混剪模式跳过关键帧提取，处理速度更快；短剧解说模式使用字幕优先管线，减少LLM调用次数。
...
- **新增** 短脚本生成问题
  - 短剧混剪模式：确保已上传有效的SRT字幕文件，检查视频文件格式。
  - 短剧解说模式：字幕内容应包含足够的剧情信息，避免过于简短的字幕。
  - 实时进度跟踪：如进度条不更新，检查浏览器控制台是否有JavaScript错误。

**章节来源**
- [ffmpeg_diagnostics.py:201-281](file://webui/components/ffmpeg_diagnostics.py#L201-L281)

## 结论
NarratoAI Web 界面提供了从脚本生成、素材选择、音频与字幕配置到视频生成与 FFmpeg 诊断的一体化工作流。通过合理配置各模块参数与利用缓存清理、硬件加速与兼容性报告，可在保证质量的同时提升生成效率。**新增的短脚本生成功能**进一步扩展了界面能力，支持多种生成模式和实时进度跟踪，为不同类型的视频制作需求提供了更灵活的解决方案。建议在正式批量生产前，先进行小规模试制与参数调优，并结合故障排除指南快速定位与解决问题。
...
- 脚本设置面板
  - **更新** 功能模式选择（文件/上传/Auto Generate/Short Generate/Short Drama Summary）。
  - **新增** 短剧混剪选项（自定义片段数量）。
  - **新增** 短剧解说字幕上传区域（支持多种格式）。
  - **新增** 实时进度跟踪（进度条和状态文本）。
  - **新增** 证据预览面板（成本估算和全局摘要）。
  - 脚本编辑区、保存脚本按钮。
- 音频设置面板
